...
//		mockMvc.perform(post("/users/save")).andExpect(status().isCreated()).andExpect(jsonPath("$[0].id", is(100001)));
//	}
	...
	public void testGetAllUsers() throws Exception {
		...
		mockMvc.perform(get("/users/all")).andExpect(status().isCreated()).andExpect(jsonPath("$[0].id", is(100001)))
	...
	public void testGetUserById() throws Exception {
		...
		mockMvc.perform(get("/users/100000")).andExpect(status().isOk()).andExpect(jsonPath("$.id", is(100000)))
				.andExpect(jsonPath("$.username", is("anil"))).andExpect(jsonPath("$.email", is("anil@gmail.com")));
