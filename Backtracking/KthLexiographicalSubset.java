...
public class KthLexiographicalSubset {
    public String getHappyString(int n, int k) {
        ArrayList<String> list = new ArrayList<>();

        fun(list, new StringBuilder(), n);
    ...
    public void fun(ArrayList<String> list, StringBuilder str, int n) {
        if (str.length() == n) {
        ...
        for (char ch : arr) {
            if (str.length() == 0 || str.charAt(str.length() - 1) != ch) {
                str.append(ch); // choose
                fun(list, str, n); // explore
